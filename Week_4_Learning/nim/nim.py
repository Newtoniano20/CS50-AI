...
class NimAI():
    ...
    def update_q_value(self, state, action, old_q, reward, future_rewards):
        ...
        if (tuple(state), tuple(action)) not in self.q:
            self.q[(tuple(state), tuple(action))] = 0
    ...
    def best_future_reward(self, state):
        ...
        if not actions:
            ...
            for action in actions:
                ...
                if action_q > best_option_q:
                    best_option = (state, action)
                    best_option_q = action_q
                elif action_q == best_option_q or best_option is None:
                    best_option = (state, action)
        if (tuple(best_option[0]), tuple(best_option[1])) not in self.q.keys():
            return 0
        return best_option_q
    ...
    def choose_action(self, state, epsilon=True):
        ...
        if not epsilon:
            ...
            for action in available_actions:
                ...
                if action_q > best_q or best_q == 0:
                    best_q = action_q
                    best_action = action
            return best_action
        else:
